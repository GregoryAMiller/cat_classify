...
class CNN(nn.Module):
    ...
    def __init__(self, dropout_pr, num_hidden, num_classes):
        ...
        self.conv2 = nn.Conv2d(1, 10, kernel_size=3)
    ...
    def forward(self, x):
        out = AF.hardtanh(self.pool1(self.conv1(x)))
        # out = AF.relu(self.pool2(self.conv2(x)))
        out = AF.hardtanh(self.pool2(self.conv2(x)))
        # out = AF.hardtanh(self.pool3(self.conv3(x)))

        out = AF.hardtanh(self.dropout_conv1(out))
        out = out.view(-1, self.num_flatten_nodes) # flattening
        out = AF.hardtanh(self.fc1(out))
        out = AF.hardtanh(self.fc2(out))
...
num_epochs = 1
alpha = 0.3    # learning rate (MLP: 0.1) (CNN: 0.3)
